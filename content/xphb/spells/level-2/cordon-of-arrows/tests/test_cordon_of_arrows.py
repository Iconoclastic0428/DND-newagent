...
class CordonOfArrowsBlockerTests(unittest.TestCase):
    SPELL_NAME = 'Cordon of Arrows'
    SPELL_SLUG = 'cordon-of-arrows'
    ...
    def test_definition_metadata_marks_the_spell_blocked(self) -> None:
        ...
        self.assertEqual(data['blocker_status'], 'needs-ward-charge-primitive')
        self.assertEqual(data['implementation_family'], 'armed-ward')

    def test_registry_metadata_marks_the_spell_blocked(self) -> None:
        data = self._read_json('registry.json')
        self.assertEqual(data['runtime_status'], 'blocked')
        self.assertEqual(data['blocker_status'], 'needs-ward-charge-primitive')
        self.assertEqual(data['test_path'], 'content/xphb/spells/level-2/cordon-of-arrows/tests/test_cordon_of_arrows.py')

    def test_readme_mentions_the_blocker(self) -> None:
        text = (ITEM_ROOT / 'README.md').read_text(encoding='utf-8')
        self.assertIn('armed ward primitive', text)
        self.assertIn('/cast player-1 cordon-of-arrows player-1', text)

    def test_implementation_note_mentions_the_blocker(self) -> None:
        text = (ITEM_ROOT / 'IMPLEMENTATION.md').read_text(encoding='utf-8')
        self.assertIn('ward-charge primitive', text)
        self.assertIn('proximity trigger', text)
