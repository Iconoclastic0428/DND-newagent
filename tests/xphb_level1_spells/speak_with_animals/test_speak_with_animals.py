...
class SpeakWithAnimalsLevel1SpellTests(StoryCantripTestCase):
    SPELL_NAME = 'Speak with Animals'
    SPELL_SLUG = 'speak-with-animals'
    ...
    def test_story_cast_intent_uses_self_targetless_syntax(self) -> None:
        ...
        intent = session._story_mode_cast_intent('/cast player-1 speak-with-animals')
    ...
    def test_cast_sets_the_beast_communication_flag(self) -> None:
        session = self._setup_session()
        self._apply_story_spell(session)
        self.assertTrue(session.state.actors['player-1'].can_communicate_with_beasts)

    def test_effect_definition_uses_ten_minute_duration_and_replacement(self) -> None:
        session = self._setup_session()
        self._apply_story_spell(session)
    ...
    def test_ending_the_effect_clears_the_beast_communication_flag(self) -> None:
        session = self._setup_session()
        self._apply_story_spell(session)
        effect = next(effect for effect in session.state.active_effects.values() if effect.name == self.SPELL_NAME)
        for event in session.encounter_session.control_runtime.kernel.effect_executor.end_active_effect(session.state, effect_id=effect.effect_instance_id, reason='test-end'):
            session.state.event_log.append(event)
            session._apply_runtime_event(event)
        self.assertFalse(session.state.actors['player-1'].can_communicate_with_beasts)

    def test_adjudication_context_mentions_beast_communication_after_the_cast(self) -> None:
        session = self._setup_session()
        self._apply_story_spell(session)
        context = session._build_adjudication_context(controller_id='player-1-controller', actor_id='player-1', declaration='Ask the hound what it saw on the road.')
        self.assertIn('can communicate with beasts', context.actor_status_summary)

    def test_adjudication_context_omits_the_beast_tag_before_the_cast(self) -> None:
        session = self._setup_session()
        context = session._build_adjudication_context(controller_id='player-1-controller', actor_id='player-1', declaration='Ask the hound what it saw on the road.')
        self.assertNotIn('can communicate with beasts', context.actor_status_summary)
