...
class FighterWeaponMasteryFeatureTests(Level1ClassFeatureTestCase):
    CHOICE_ID = 'class:fighter:weapon-mastery'

    def _record(self):
        return self.complete_record('fighter', overrides={self.CHOICE_ID: ('greatsword', 'flail', 'javelin')})
    ...
    def test_selected_weapon_masteries_persist_in_the_record(self) -> None:
        record = self._record()
        self.assertEqual(record.weapon_mastery_item_ids, ('greatsword', 'flail', 'javelin'))
    ...
    def test_selected_mastery_items_compile_into_known_attacks(self) -> None:
        ...
        self.assertTrue({'greatsword', 'flail', 'javelin'}.issubset(attack_item_ids))
