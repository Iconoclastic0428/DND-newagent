...
class BarbarianWeaponMasteryFeatureTests(Level1ClassFeatureTestCase):
    CHOICE_ID = 'class:barbarian:weapon-mastery'

    def _record(self):
        return self.complete_record('barbarian', overrides={self.CHOICE_ID: ('greataxe', 'handaxe')})
    ...
    def test_selected_weapon_masteries_persist_in_the_record(self) -> None:
        record = self._record()
        self.assertEqual(record.weapon_mastery_item_ids, ('greataxe', 'handaxe'))
    ...
    def test_selected_mastery_items_compile_into_known_attacks(self) -> None:
        ...
        self.assertTrue({'greataxe', 'handaxe'}.issubset(attack_item_ids))
