...
class RangerWeaponMasteryFeatureTests(Level1ClassFeatureTestCase):
    CHOICE_ID = 'class:ranger:weapon-mastery'

    def _record(self):
        return self.complete_record('ranger', overrides={self.CHOICE_ID: ('shortsword', 'longbow')})
    ...
    def test_selected_weapon_masteries_persist_in_the_record(self) -> None:
        record = self._record()
        self.assertEqual(record.weapon_mastery_item_ids, ('shortsword', 'longbow'))
    ...
    def test_selected_mastery_items_compile_into_known_attacks(self) -> None:
        ...
        self.assertTrue({'shortsword', 'longbow'}.issubset(attack_item_ids))
