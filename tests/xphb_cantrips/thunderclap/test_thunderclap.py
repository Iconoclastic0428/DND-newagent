...
class ThunderclapCantripTests(EncounterCantripTestCase):
    def _setup_session(self):
        session = self.build_default_session()
        self.grant_spell(session, 'Thunderclap')
        self.advance_to_actor(session, 'player-1')
        session.state.actors['player-1'].spell_save_dc = 99
        return session

    def test_failed_save_deals_thunder_damage(self) -> None:
        ...
        session.state, _ = session.command_interface.execute(session.state, '/cast player-1 thunderclap')
