...
This trainer splits command prediction into a command-family head, a natural-action fallback head, an argument-count head, and slash-command argument heads. Combat command-family and argument decoding are constrained by the row's available commands when possible, so predicted command types and actor, weapon, or target slots stay inside the locally legal action set. It is still a local preflight; it does not submit a MosaicML job.
